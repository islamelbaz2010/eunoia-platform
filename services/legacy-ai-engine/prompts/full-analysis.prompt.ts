...
Return ONLY valid JSON (no markdown, no backticks, start directly with {):
{
  ...
  "proposal": [
    {"title": "Full Marketing Partnership — ${ctx.branch.name}", "desc": "Complete execution: Meta + Google + TikTok management, content production, weekly strategy calls, monthly full report, CRM setup", "price": "EGP ${sector.en.toLowerCase().includes('real estate') || sector.en.toLowerCase().includes('developer') ? '25,000–45,000' : '8,000–40,000'}/month", "best_for": "Serious growth with full-service execution", "reasoning": "Priced at ${sector.en} market rate for ${ctx.branch.name} — includes 3 specialists minimum"},
    {"title": "Performance Growth Package", "desc": "Paid campaigns + content + monthly analysis — focused on lead generation", "price": "EGP ${sector.en.toLowerCase().includes('real estate') ? '15,000–28,000' : '6,000–18,000'}/month", "best_for": "Growing business with proven offer needing scale", "reasoning": "Covers media buyer + strategist + monthly report — no content production team"},
