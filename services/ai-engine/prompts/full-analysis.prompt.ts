...
Return ONLY valid JSON (no markdown, no backticks, start directly with {):
{
  ...
  "proposal": [
    {"title": "Full Marketing Partnership — ${ctx.branch.name}", "desc": "Complete execution: Meta + Google + TikTok management, content production, weekly strategy calls, monthly full report, CRM setup", "price": "EGP ${sector.en.toLowerCase().includes('real estate') || sector.en.toLowerCase().includes('developer') ? '25,000–45,000' : size === 'enterprise' ? '20,000–40,000' : size === 'large' ? '12,000–22,000' : size === 'medium' ? '8,000–15,000' : '5,000–10,000'}/month", "best_for": "Serious growth with full-service execution", "reasoning": "Priced at ${sector.en} market rate for ${ctx.branch.name} — includes 3 specialists minimum"},
    {"title": "Performance Growth Package", "desc": "Paid campaigns + content + monthly analysis — focused on lead generation", "price": "EGP ${sector.en.toLowerCase().includes('real estate') ? '15,000–28,000' : size === 'large' || size === 'enterprise' ? '10,000–18,000' : '6,000–12,000'}/month", "best_for": "Growing business with proven offer needing scale", "reasoning": "Covers media buyer + strategist + monthly report — no content production team"},
    {"title": "Strategy + Intelligence Only", "desc": "Monthly strategy session + full intelligence report + ad account audit — for businesses with in-house execution", "price": "EGP 3,500–7,000/month", "best_for": "Business with internal team needing senior guidance", "reasoning": "2 senior hours/month + full report + audit — positioning value not time cost"}
  ],
  "why_us": [
